        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            ...
            <div class="bg-white dark:bg-gray-700 rounded-lg shadow p-6">
              ...
              <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.users?.active || 0 }}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">Active users</p>
            </div>
          ...
          <div v-if="selectedUserDetails" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white dark:bg-gray-700 rounded-lg shadow-lg max-w-4xl max-h-96 overflow-y-auto w-full mx-4 p-6">
              ...
              <div class="mb-4">
                <h3 class="font-bold text-gray-900 dark:text-white mb-2">Recent Income</h3>
                <div class="space-y-1 text-sm max-h-32 overflow-y-auto">
                  <div v-for="income in selectedUserDetails.income?.slice(0, 5)" :key="income.id" class="flex justify-between text-gray-600 dark:text-gray-400">
                    <span>{{ income.category_name }} - {{ income.description }}</span>
                    <span>+{{ formatCurrency(income.amount) }}</span>
                  </div>
                  <div v-if="selectedUserDetails.income?.length === 0" class="text-gray-500 dark:text-gray-400">No income records</div>
                </div>
              </div>

              <div class="mb-4">
                <h3 class="font-bold text-gray-900 dark:text-white mb-2">Recent Transfers</h3>
                <div class="space-y-1 text-sm max-h-32 overflow-y-auto">
                  <div v-for="transfer in selectedUserDetails.transfers?.slice(0, 5)" :key="transfer.id" class="flex justify-between text-gray-600 dark:text-gray-400">
                    <span>{{ transfer.from_account_name }} → {{ transfer.to_account_name }}</span>
                    <span>{{ formatCurrency(transfer.amount) }}</span>
                  </div>
                  <div v-if="selectedUserDetails.transfers?.length === 0" class="text-gray-500 dark:text-gray-400">No transfers</div>
                </div>
              </div>
            </div>
          </div>
                ...
                <table class="w-full text-sm">
                  ...
                  <tbody>
                    <tr v-for="transfer in allTransfers" :key="transfer.id" class="border-b border-gray-100 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600">
                      ...
                      <td class="px-4 py-2 text-gray-600 dark:text-gray-400 text-xs">{{ formatDate(transfer.transfer_date) }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
...
const toggleUserRole = async (user) => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      // Refresh the user list to ensure real-time data
      await refreshUsers()
    }
  } catch (error) {
