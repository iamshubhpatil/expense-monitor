      ...
      <main class="flex-1 overflow-y-auto bg-gray-50 dark:bg-gray-800 p-6">
        <div class="space-y-6">
          ...
          <div>
            ...
            <div v-if="errorMessage" class="mb-4 rounded-lg border border-red-300 bg-red-50 p-4 text-sm text-red-700 dark:border-red-700 dark:bg-red-900/20 dark:text-red-200">
              {{ errorMessage }}
            </div>
          </div>
...
const generateReports = async () => {
  try {
    ...
    if (filters.value.type === 'expenses') {
      categories = allCategories.filter((cat) => cat.type === 'expense')
    } else if (filters.value.type === 'income') {
      categories = allCategories.filter((cat) => cat.type === 'income')
    }

    let totalExpenses = allCategories
      .filter((cat) => cat.type === 'expense')
      .reduce((sum, cat) => sum + (Number(cat.total) || 0), 0)
    let totalIncome = allCategories
    ...
    errorMessage.value = error?.response?.data?.message || error?.response?.data?.error || 'Failed to load reports. Verify Supabase report functions are deployed and accessible.'
  } finally {
    loading.value = false
  }
}
