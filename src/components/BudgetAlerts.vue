<template>
  <div class="relative">
    ...
    <div
      v-if="showDropdown"
      class="fixed mt-2 w-96 bg-white dark:bg-gray-800 rounded-lg shadow-lg z-50 max-h-96 overflow-y-auto"
      :style="dropdownStyle"
    >
      <!-- Header -->
          ...
          <div class="flex justify-between items-start">
            <div class="flex-1">
              <div class="flex items-center space-x-2">
                ...
                >
                  {{ isExceeded(alert) ? 'Budget Exceeded' : 'Budget Warning' }}
                </p>
              </div>
...
export default {
  ...
  methods: {
    ...
    async fetchAlerts() {
      try {
        const response = await apiClient.get('/budgets/alerts/list');
    ...
    updateDropdownPosition() {
      if (!this.$refs.bellButton || typeof window === 'undefined') return
    ...
    isExceeded(alert) {
      return String(alert.alert_type || '').toLowerCase() === 'exceeded'
    },
    async handleAlertClick(alert) {
      if (!alert.is_read) {
        try {
          await apiClient.put('/budgets/alerts/read', { id: alert.id });
